...
<page class="overflow-y-auto">
	...
	<main class="mx-6 my-10 flex min-h-[80vh] flex-col gap-9 items-center justify-center">
		...
		<!-- Registration form -->
		<form method="POST" use:enhance class="flex flex-col w-full gap-8 lg:px-12">
			<div class="grid w-full items-center gap-1.5">
				<Label for="name" class="w-fit">Full Name</Label>
				<p class="text-sm text-destructive">{$errors.name}</p>
				<Input type="text" bind:value={$form.name} name="name" placeholder="Enter your full name" class="w-full" {...$constraints.name} />
			</div>
			<div class="grid w-full items-center gap-1.5">
				<Label for="email" class="w-fit">Email</Label>
				<p class="text-sm text-destructive">{$errors.email}</p>
				<Input type="email" bind:value={$form.email} name="email" placeholder="Enter your email" class="w-full" {...$constraints.email} />
			</div>
			<div class="grid w-full items-center gap-1.5">
				<Label for="password" class="w-fit">Password</Label>
				<p class="text-sm text-destructive">{$errors.password}</p>
				<Input type="password" bind:value={$form.password} name="password" placeholder="Create a password" class="w-full" {...$constraints.password} />
			</div>
			<div class="grid w-full items-center gap-1.5">
				<Label for="confirmPassword" class="w-fit">Confirm Password</Label>
				<p class="text-sm text-destructive">{$errors.confirmPassword}</p>
				<Input type="password" bind:value={$form.confirmPassword} name="confirmPassword" placeholder="Confirm your password" class="w-full" {...$constraints.confirmPassword} />
			</div>
			<p class="text-sm text-destructive">{$errors._errors}</p>
			<p class="text-sm text-green-400">{$message}</p>
			<Button type="submit" class="w-full">Create Account</Button>
			<div class="flex w-full items-center justify-center">
				<a href="/auth/login" class="text-sm text-primary underline">Already have an account? Log in</a>
			</div>
		</form>
	</main>
</page>
