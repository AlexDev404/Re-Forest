	...
	<main class="mx-6 my-10 flex min-h-[80vh] flex-col gap-9 items-center justify-center">
		...
		<form method="POST" use:enhance class="flex flex-col w-full gap-8 lg:px-12">
			...
			<div class="grid w-full items-center gap-1.5">
				...
				<Input type="password" bind:value={$form.password} name="password" placeholder="Enter your password" class="w-full" {...$constraints.password} />
			</div>
			<p class="text-sm text-destructive">{$errors._errors}</p>
			<p class="text-sm text-green-400">{$message}</p>
			<Button type="submit" class="w-full">Login</Button>
			<div class="flex w-full items-center justify-center">
				<a href="/auth/register" class="text-sm text-primary underline">Create an account</a>
			</div>
		</form>
	</main>
</page>
