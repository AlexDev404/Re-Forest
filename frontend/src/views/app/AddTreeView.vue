...
import { treesAdapter, type PlantingReason, type TreeSpeciesData } from '@/adapters/trees';
...
async function querySpecies(query: string = '') {
  try {
    ...
    const data = await treesAdapter.getSpecies({ q: query, limit: 10, is_timber: isTimberParam });
...
async function useCurrentLocationFn() {
  try {
    ...
    if (loc) {
      ...
      location.value = loc;
      const reverse: ReverseGeoJSON | null = await getReverseLoc(loc.latitude, loc.longitude);
      if (reverse) {
...
function addTreeToBatch() {
  ...
  const entry: TreeEntry = {
    id: editingEntryId.value || `tree-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
    tree_name: treeName.value,
  ...
  if (editingEntryId.value) {
    const idx = treeEntries.value.findIndex((e) => e.id === editingEntryId.value);
...
<template>
  <div class="block min-h-screen overflow-y-auto bg-slate-50 pb-24 font-sans dark:bg-slate-900">
    <main class="mx-auto flex w-full max-w-4xl flex-col gap-10 px-4 py-10 sm:px-6 sm:py-16">
      <header class="flex flex-col items-start gap-2.5 pb-8">
        <h1 class="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">Plant a New Tree</h1>
        <p class="text-sm text-muted-foreground">Provide details about your tree planting activity.</p>
      </header>

      <!-- Planter Type Selection -->
      <div v-if="!planterType" class="flex flex-col gap-6 rounded-xl border border-border bg-card p-8 shadow">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <button type="button" @click="planterType = 'INDIVIDUAL'; planterTypeField = 'INDIVIDUAL'"
            ...
            <div class="flex h-16 w-16 items-center justify-center rounded-full bg-primary/10 transition-colors group-hover:bg-primary/20">
          ...
          <button type="button" @click="planterType = 'ORGANIZATION'; planterTypeField = 'ORGANIZATION'"
            class="group flex cursor-pointer flex-col items-center gap-4 rounded-lg border-2 border-border bg-background p-6 transition-all duration-200 hover:border-primary hover:bg-primary/5">
            <div class="flex h-16 w-16 items-center justify-center rounded-full bg-primary/10 transition-colors group-hover:bg-primary/20">
      ...
      <template v-if="planterType">
        ...
        <div class="flex items-center justify-between rounded-xl border border-border bg-card p-4 shadow">
          <div class="flex items-center gap-3">
            <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10">
              <User v-if="planterType === 'INDIVIDUAL'" class="h-5 w-5 text-primary" />
              <Building2 v-else class="h-5 w-5 text-primary" />
            </div>
            <div>
              <p class="text-sm font-medium text-foreground">Planting as {{ planterType === 'INDIVIDUAL' ? 'Individual' : 'Organization' }}</p>
              <p class="text-xs text-muted-foreground">{{ planterType === 'INDIVIDUAL' ? 'Personal tree planting effort' : 'Organization or group planting effort' }}</p>
            </div>
          </div>
          <Button variant="ghost" size="sm" @click="planterType = null" class="text-xs">Change</Button>
        </div>

        <!-- Image Upload Card -->
        <div class="flex flex-col items-center gap-6 rounded-xl border border-border bg-card p-8 text-center shadow">
          <div class="mb-2 flex h-28 w-28 items-center justify-center overflow-hidden rounded-full border-2 border-background bg-muted shadow-inner transition-all duration-300 sm:h-32 sm:w-32"
            :class="treeImageSrc ? 'ring-2 ring-primary ring-offset-2 ring-offset-background' : 'ring-1 ring-border'">
            <img v-if="treeImageSrc" :src="treeImageSrc" alt="Tree preview" class="h-full w-full object-cover" />
            <ImageUp v-else class="h-10 w-10 text-muted-foreground opacity-60 sm:h-12 sm:w-12" />
          </div>
          ...
          <Button variant="outline" @click="handlePhotoClick" :disabled="uploading"
            ...
            {{ uploading ? 'Processing...' : treeImageSrc ? 'Update Photo' : 'Upload Photo' }}
          </Button>
          <p v-if="formErrors.tree_image" class="mt-1 flex items-center gap-1 text-xs text-destructive">
            <AlertTriangle class="h-3.5 w-3.5" /> {{ formErrors.tree_image }}
          </p>
        </div>

        <!-- Form -->
        <form @submit.prevent="submitSingleTree" class="flex w-full flex-col gap-6 rounded-xl border border-border bg-card p-8 shadow sm:p-10">
          <!-- Organization Name -->
          <div v-if="planterType === 'ORGANIZATION'" class="grid w-full items-center gap-2">
            <Label for="organization_name" class="flex items-center gap-1.5 text-sm font-medium text-foreground">
              <Building2 class="h-4 w-4 text-primary/80" /> Organization Name <span class="text-xs text-destructive">*</span>
            </Label>
            <Input type="text" id="organization_name" v-model="organizationName" placeholder="e.g., Belize Forest Department, Green Earth NGO" class="w-full" :required="planterType === 'ORGANIZATION'" />
          </div>

          <!-- Tree Name -->
          <div class="grid w-full items-center gap-2">
            <Label for="tree_name" class="flex items-center gap-1.5 text-sm font-medium text-foreground">
            ...
            <Input type="text" id="tree_name" v-model="treeName" :placeholder="planterType === 'INDIVIDUAL' ? 'e.g., Mahogany Tree, Coconut Palm' : 'e.g., Reforestation Project 2026'" class="w-full" required @input="saveFormState" />
            <p v-if="formErrors.tree_name" class="flex items-center gap-1 text-xs text-destructive">
              <AlertTriangle class="h-3.5 w-3.5" /> {{ formErrors.tree_name }}
            </p>
          </div>

          <!-- Quantity / Area -->
          <div v-if="planterType === 'INDIVIDUAL'" class="grid w-full items-center gap-2">
            <Label for="quantity" class="flex items-center gap-1.5 text-sm font-medium text-foreground">Quantity (number of trees/plants)</Label>
            <Input type="number" id="quantity" v-model="quantity" placeholder="e.g., 1, 50" class="w-full" min="1" />
            <p class="text-xs text-muted-foreground">If planting multiple trees of the same type, enter the quantity here.</p>
          </div>
          <div v-else class="grid w-full items-center gap-2">
            <Label for="area_hectares" class="flex items-center gap-1.5 text-sm font-medium text-foreground">Area (hectares)</Label>
            <Input type="number" id="area_hectares" v-model="areaHectares" placeholder="e.g., 2.5" class="w-full" min="0.01" step="0.01" />
            <p class="text-xs text-muted-foreground">1 hectare = 2.47 acres. Enter the approximate area being planted/reforested.</p>
          </div>

          <!-- Height -->
          <div class="grid w-full items-center gap-2">
            <Label for="tree_height" class="flex items-center gap-1.5 text-sm font-medium text-foreground">
              Height (feet) <span class="text-xs text-muted-foreground">(optional)</span>
            </Label>
            <Input type="number" id="tree_height" v-model="treeHeight" placeholder="e.g., 1.5" class="w-full" min="0.1" step="0.01" />
          </div>
          ...
          <div class="grid w-full items-center gap-2">
            ...
            <div v-if="selectedSpeciesName" class="mb-1 flex items-center justify-between rounded-md border border-input bg-muted/50 p-2.5 shadow-sm">
              <div class="flex items-center gap-2 text-foreground">
                <CheckCircle2 class="h-4 w-4 text-green-600" />
                <span class="text-sm font-medium">{{ selectedSpeciesName }}</span>
              </div>
              <Button variant="ghost" size="sm" class="h-7 px-2 text-xs" @click="selectedSpeciesName = null; treeSpecies = ''; querySpecies()">
                <X class="mr-1 h-3.5 w-3.5" /> Change
              </Button>
            </div>

            <!-- Species search -->
            <div v-if="!selectedSpeciesName" class="rounded-md border border-input bg-background shadow-sm">
              <input type="text" placeholder="Search and select species..."
                class="h-9 w-full border-0 bg-transparent px-3 text-sm placeholder-muted-foreground/60 focus:outline-none focus:ring-0"
                @input="(e: Event) => handleSpeciesSearch((e.target as HTMLInputElement).value)" />
              <div v-if="speciesData && speciesData.length > 0" class="max-h-[180px] overflow-y-auto border-t border-input p-1">
                <button v-for="sp in speciesData" :key="sp.id" type="button"
                  class="flex w-full cursor-pointer items-center rounded p-2 text-sm text-foreground transition-colors hover:bg-muted"
                  @click="treeSpecies = sp.id.toString(); selectedSpeciesName = sp.name">
                  <Leaf class="mr-2 h-3.5 w-3.5 text-primary/70" />
                  <span class="text-xs">{{ sp.name }}</span>
                </button>
              </div>
              <div v-else-if="speciesData && speciesData.length === 0" class="p-3 text-center text-xs text-muted-foreground">No species found.</div>
            </div>
            <p v-if="formErrors.tree_species" class="mt-0.5 flex items-center gap-1 text-xs text-destructive">
              <AlertTriangle class="h-3.5 w-3.5" /> {{ formErrors.tree_species }}
            </p>
          </div>

          <!-- Planting Reason -->
          <div class="grid w-full items-center gap-3">
            <Label class="flex items-center gap-1.5 text-sm font-medium text-foreground">Reason for Planting</Label>
            <div class="flex flex-col gap-2">
              <div v-for="reason in plantingReasons" :key="reason.Id"
                ...
                <div class="aspect-square h-4 w-4 rounded-full border border-primary"
                  :class="plantingReasonId === reason.Id.toString() ? 'bg-primary' : ''"></div>
                <label class="flex-1 cursor-pointer text-sm font-normal leading-relaxed">{{ reason.Name }}</label>
              </div>
            </div>
          ...
          <!-- Hashtags -->
          <div class="grid w-full items-center gap-2">
            <Label for="hashtags" class="flex items-center gap-1.5 text-sm font-medium text-foreground">
              Tags <span class="text-xs text-muted-foreground">(optional)</span>
            </Label>
            <Input type="text" id="hashtags" v-model="hashtags" placeholder="e.g., #climateaction #reforestation #belize" class="w-full" />
            <p class="text-xs text-muted-foreground">Add relevant tags to categorize your planting.</p>
          </div>

          <!-- Location -->
          <div class="grid w-full items-center gap-3 rounded-xl border border-border bg-muted/30 p-6 shadow dark:bg-slate-800/30">
            <Label class="flex items-center gap-1.5 text-sm font-medium text-foreground">
              <MapPin class="h-4 w-4 text-primary/80" /> Planting Location
            </Label>
            <template v-if="translatedLocation">
              <p class="text-xs text-muted-foreground">
                Current: <strong class="font-medium text-foreground">{{ translatedLocation }}</strong>
              </p>
              <p v-if="location" class="text-xs text-muted-foreground/70">
                (Lat: {{ location.latitude.toFixed(5) }}, Lng: {{ location.longitude.toFixed(5) }})
              </p>
            </template>
            <p v-else class="flex items-center gap-1.5 rounded-md border border-orange-200 bg-orange-50 p-2.5 text-xs text-orange-600 dark:border-orange-500/30 dark:bg-orange-900/30 dark:text-orange-400">
              <AlertTriangle class="h-4 w-4" /> Location not set. Please specify location.
            </p>
            <Button type="button" variant="outline" @click="openMapPicker" class="mt-1 w-full sm:w-auto">
              <MapPin class="mr-1.5 h-4 w-4" /> {{ translatedLocation ? 'Modify Location' : 'Set Location' }}
            </Button>
            <Button type="button" variant="outline" :disabled="!!translatedLocation" @click="useCurrentLocationFn" class="w-full sm:w-auto">
              <Locate class="mr-1.5 h-4 w-4" /> Use Current Location
            </Button>
          </div>

          <!-- Error display -->
          <p v-if="generalError" class="flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm font-medium text-destructive">
            <AlertTriangle class="h-5 w-5" /> {{ generalError }}
          </p>

          <!-- Submit buttons -->
          <div class="flex flex-col gap-3 sm:flex-row">
            <Button type="submit" :disabled="uploading || !location || !planterType"
            ...
            <Button type="button" variant="outline" @click="addTreeToBatch"
              :disabled="uploading || !location || !planterType || !treeName || !treeSpecies || !treeImage"
          ...
          <div v-if="treeEntries.length > 0" class="mt-2 text-center text-xs text-muted-foreground">
            Or add multiple trees to a batch and submit all at once
          </div>
        </form>

        <!-- Batch Tree Entries -->
        <div v-if="treeEntries.length > 0" class="rounded-xl border border-border bg-card p-6 shadow">
          <div class="mb-4 flex items-center justify-between">
            <div class="flex items-center gap-2">
              <List class="h-5 w-5 text-primary" />
              <h2 class="text-lg font-semibold text-foreground">Batch Tree Entries ({{ treeEntries.length }})</h2>
            </div>
            <Button @click="submitAllTrees" :disabled="batchSubmitting" class="bg-primary text-primary-foreground hover:bg-primary/90">
              <UploadCloud v-if="batchSubmitting" class="mr-2 h-4 w-4 animate-spin" />
              <CheckCircle2 v-else class="mr-2 h-4 w-4" />
              {{ batchSubmitting ? 'Submitting...' : `Submit All ${treeEntries.length} Trees` }}
            </Button>
          </div>

          <div class="space-y-3">
            <div v-for="(entry, index) in treeEntries" :key="entry.id"
              ...
              <div class="flex-1">
                <div class="mb-1 flex items-start justify-between">
                  ...
                  <span class="rounded-full bg-primary/10 px-2 py-1 text-xs font-medium text-primary">#{{ index + 1 }}</span>
                </div>
                <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  <span v-if="entry.tree_height">Height: {{ entry.tree_height }}m</span>
                  <span v-if="entry.tree_age">Age: {{ entry.tree_age }} years</span>
                  <span v-if="entry.quantity">Qty: {{ entry.quantity }}</span>
                  <span v-if="entry.area_hectares">Area: {{ entry.area_hectares }} ha</span>
                </div>
              </div>
              <div class="flex flex-col gap-2">
                <Button size="sm" variant="ghost" @click="editEntry(entry.id)" class="h-8 w-8 p-0">
                  <Edit class="h-4 w-4" />
                </Button>
                <Button size="sm" variant="ghost" @click="removeEntry(entry.id)" class="h-8 w-8 p-0 text-destructive hover:bg-destructive/10">
          ...
          <div v-if="batchErrorMessage" class="mt-4 flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
