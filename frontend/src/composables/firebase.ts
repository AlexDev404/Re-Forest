...
import { notificationAdapter } from '@/adapters/notifications';
  ...
  if (await isSupported()) {
    ...
    onMessage(messaging, (payload) => {
      ...
      if (Notification.permission === 'granted') {
        new Notification(title || 'New Notification', { body, icon });
      }
    });
...
export async function requestNotificationPermission() {
  try {
    ...
    // Send to backend
    await notificationAdapter.saveFcmToken(token, navigator.userAgent);
